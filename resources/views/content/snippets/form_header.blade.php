@if($submission)
  <div class="card-header sticky-element bg-label-warning d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row mb-5">
    ...
    <div class="action-btns">
      @if($submission->status == 'approved' || $submission->status == 'denied' || $submission->status == 'filed')
        @can('print '. $form_name)
      ...
      @can('edit '. $form_name)
        @if(($submission->status == 'approved' || $submission->status == 'filed')&& \Illuminate\Support\Facades\Auth::user()->can('edit '.strtolower($submission->name)) )
